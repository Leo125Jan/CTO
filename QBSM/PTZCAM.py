...
class PTZcon():
	...
	def UpdateState(self, targets, neighbors, one_hop_neighbor,
					Pd, cluster_set, col_ind, cluster_labels, cluster_centers, 
					time_, cp, speed_gain, save_type, Times, target_speed):
					...
					if len(neighbor.comc[str(self.id)]) != 0:
						...
						for element in neighbor.comc[str(self.id)]:
							...
							if element not in path:
								...
								path = np.concatenate((path, [element]), axis = 0)
	...
	def Gradient_Ascent(self, targets, time_):
		...
		P1_gain = min(1.0 + 2*np.exp(dist_s-3.0), 3)
		P2_gain = min(3.0*dist_s, 3)
		if dist <= range_max:
	...
	def CBF_Collision_Herding(self, targets):
		...
		dt_min = 0.5*self.incircle_r
		da_min = 0.5*self.incircle_r
		G, H = [], []
		...
		for i in range(len(targets)):
			...
			if dist < communication_range:
				...
				print("target pose: ", targets[i][0])
				print("target speed: ", self.target_speed[i])

				d = (self.pos - targets[i][0])
				h = 1.0*(dist**2 - dt_min**2) - 2*np.dot(d, self.target_speed[i])
		...
		for neighbor in self.neighbors:
			...
			if dist < communication_range:
				...
				# print("neighbor speed: ", neighbor.translational_force)

				d = (self.pos - neighbor.pos)
				h = 1.0*(dist**2 - da_min**2) - 2*np.dot(d, neighbor.translational_force)
				G.append([-2*d[0], -2*d[1]])
				H.append(h)
		...
		if len(G) > 0 and len(H) > 0:
			...
			try:
				...
				gain = min(control_input_norm, 2.0)
	...
	def UpdateOrientation(self):
		...
		print("perspective_force: ", self.perspective_force)
