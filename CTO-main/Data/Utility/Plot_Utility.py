...
def Execute_read():
	...
	for i in range(np.shape(Data)[0]):
		...
		globals()["x_" + str(i)] = Data[i][1]
		globals()["y_" + str(i)] = Data[i][0]

	init()

	ax0.plot(x_0, y_0, color = "#FF0000")
	ax0.set_title('Utility of Agent 0 (Red)', fontdict = {'fontsize': 20})
	ax0.set_xlabel("Time (s)", fontdict = {'fontsize': 20})
	ax0.set_ylabel("Utility", fontdict = {'fontsize': 20})
	ax0.tick_params(axis='both', which='major', labelsize = 20)

	ax1.plot(x_1, y_1, color = "#00BB00")
	ax1.set_title('Utility of Agent 1 (Green)', fontdict = {'fontsize': 20})
	ax1.set_xlabel("Time (s)", fontdict = {'fontsize': 20})
	ax1.set_ylabel("Utility", fontdict = {'fontsize': 20})
	ax1.tick_params(axis='both', which='major', labelsize = 20)

	ax2.plot(x_2, y_2, color = "#0072E3")
	ax2.set_title('Utility of Agent 2 (Blue)', fontdict = {'fontsize': 20})
	ax2.set_xlabel("Time (s)", fontdict = {'fontsize': 20})
	ax2.set_ylabel("Utility", fontdict = {'fontsize': 20})
	ax2.tick_params(axis='both', which='major', labelsize = 20)

	ax3.plot(x_3, y_3, color = "#FF9500")
	ax3.set_title('Utility of Agent 3 (Orange)', fontdict = {'fontsize': 20})
	ax3.set_xlabel("Time (s)", fontdict = {'fontsize': 20})
	ax3.set_ylabel("Utility", fontdict = {'fontsize': 20})
