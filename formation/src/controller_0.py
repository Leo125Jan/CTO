...
def	controller():
	...
	# Position Control
	cmd_vel.linear.x = 0.5*((P1[0] - P0[0]) + 8.0 + (P2[0] - P0[0]) + 4.0) + 0.5*(P3[0] - P0[0] + 4.0) + 1*P3_v[0]
	cmd_vel.linear.y = 0.5*((P1[1] - P0[1]) + 0.0 + (P2[1] - P0[1]) - 4.0*sqrt(3)) + 0.5*(P3[1] - P0[1] - 2.0*sqrt(3)) + 1*P3_v[1]
	cmd_vel.linear.z = 1.0 - P0[2]
	...
	perspective_t = np.array([P3[0] - P0[0], P3[1] - P0[1]]);
...
def Orbit(t):
	...
	# Position Control
	cmd_vel.linear.x = 5*np.cos(t) - P0[0]
	cmd_vel.linear.y = 5*np.sin(t) - P0[1]
	cmd_vel.linear.z = 1.6 - P0[2]
	...
	cmd_vel.angular.z = 2.0*dtheta*axis
...
if __name__ == '__main__':
	...
	try:
		...
		px4_0 = Px4Controller("uav0")
		rospy.Subscriber('/gazebo/model_states', ModelStates, odom, queue_size = 10)
		rospy.Subscriber("/uav0/mavros/imu/data", Imu, imu_callback, queue_size = 10)
