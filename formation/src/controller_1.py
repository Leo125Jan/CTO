...
def	controller():
	...
	# Position Control
	cmd_vel.linear.x = 0.5*((P0[0] - P1[0]) - 8.0 + (P2[0] - P1[0]) - 4.0) + 0.5*(P3[0] - P1[0] - 4.0) + 1*P3_v[0]
	cmd_vel.linear.y = 0.5*((P0[1] - P1[1]) + 0.0 + (P2[1] - P1[1]) - 4.0*sqrt(3)) + 0.5*(P3[1] - P1[1] - 2.0*sqrt(3)) + 1*P3_v[1]
	cmd_vel.linear.z = 1.0 - P1[2]

	# Heading Control
	theta = current_heading
	perspective = np.array([1*cos(theta), 1*sin(theta)])

	perspective_t = np.array([P3[0] - P1[0], P3[1] - P1[1]]);
...
if __name__ == '__main__':
	...
	try:
		...
		px4_1 = Px4Controller("uav1")
		pose_sub = rospy.Subscriber('/gazebo/model_states', ModelStates, odom, queue_size = 10)
		imu_sub = rospy.Subscriber("/uav1/mavros/imu/data", Imu, imu_callback, queue_size = 10)
