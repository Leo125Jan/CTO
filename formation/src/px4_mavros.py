...
if __name__ == '__main__':
    try:
        ...
        uavtype = ["uav0","uav1","uav2","uav3"]

        px4_0 = Px4Controller(uavtype[0])
        px4_1 = Px4Controller(uavtype[1])
        px4_2 = Px4Controller(uavtype[2])
        px4_3 = Px4Controller(uavtype[3])

        # px4_0.OffboardandArm()
        # px4_1.OffboardandArm()
        # px4_2.OffboardandArm()
        # px4_3.OffboardandArm()

        last_time_0 = rospy.Time.now()
        last_time_1 = rospy.Time.now()
        last_time_2 = rospy.Time.now()
        last_time_3 = rospy.Time.now()

        while not rospy.is_shutdown():
            if rospy.Time.now() - last_time_0 > rospy.Duration(10):
                if not px4_0.current_state.armed or px4_0.current_state.mode != "OFFBOARD":
                	px4_0.OffboardandArm()
                	last_time_0 = rospy.Time.now()
