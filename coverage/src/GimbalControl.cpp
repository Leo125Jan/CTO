...
GimbalControl::GimbalControl(std::string self_name, std::string target_name)
{
    self_name = self_name;
    target_name = target_name;
    // car_odom_sub = nh.subscribe<nav_msgs::Odometry>
    //         ("/uav5/base_pose_ground_truth", 10, &GimbalControl::getAgentOdom, this);
    car_odom_sub = nh.subscribe<nav_msgs::Odometry>
            (target_name, 10, &GimbalControl::getAgentOdom, this);
    ...
    fw_pose_sub = nh.subscribe<nav_msgs::Odometry>
            ("gimbal/base_pose_ground_truth", 10, &GimbalControl::getFwPose, this);

    // state_sub = nh.subscribe<gazebo_msgs::ModelStates>("/gazebo/model_states", 10, &GimbalControl::state_CB, this);
    link_sub = nh.subscribe<gazebo_msgs::LinkStates>("/gazebo/link_states", 10, &GimbalControl::link_CB, this);
    ...
    cameraInfo_sub = nh.subscribe<sensor_msgs::CameraInfo>
            ("camera/camera/color/camera_info", 10, &GimbalControl::getCameraInfo, this);
    ...
    /*** UKF ***/
    estimate_dyn_pub = nh.advertise<coverage::Dynamics>
            ("estimation/ukf/dynamics", 10);
    estimate_cam_pub = nh.advertise<coverage::CameraFeatures>
            ("estimation/ukf/camera_features", 10);
    ukf_sub = nh.subscribe<coverage::EstimateOutput>
...
void GimbalControl::state_CB(const gazebo_msgs::ModelStates::ConstPtr& msg) 
{
    gazebo_msgs::ModelStates state_ = *msg;
    
    auto it_self = find(state_.name.begin(), state_.name.end(), self_name);
    auto it_target = find(state_.name.begin(), state_.name.end(), target_name);
    int index_self, index_target;

    // If element was found
    if (it_self != state_.name.end())
    {
        index_self = it_self - state_.name.begin();
    }
    else
    {
    ...
    // If element was found
    if (it_target != state_.name.end())
    {
        index_target = it_target - state_.name.begin();
    }
    else
    {
        // If the element is not present in the array
    ...
    /*Car*/
    carVel << state_.twist[index_target].linear.x, state_.twist[index_target].linear.y, state_.twist[index_target].linear.z;
    carPos = state_.pose[index_target];

    // Odom
    fwAngVel << state_.twist[index_self].angular.x, state_.twist[index_self].angular.y, state_.twist[index_self].angular.z;
    // frame: flu -> frd
    fwAngVel = R_frd * fwAngVel;

    // Pose
    fwPos = state_.pose[index_self];
    fwVel << state_.twist[index_self].linear.x, state_.twist[index_self].linear.y, state_.twist[index_self].linear.z;

    quat_planeEarth_flu = Eigen::Quaternionf(state_.pose[index_self].orientation.w, state_.pose[index_self].orientation.x, state_.pose[index_self].orientation.y, state_.pose[index_self].orientation.z );
    R_planeEarth_frd =  R_frd * Quat2RotaMatrix(quat_planeEarth_flu);
}

void GimbalControl::link_CB(const gazebo_msgs::LinkStates::ConstPtr& msg) 
{
    float ref = 0.0;
    angRoll = gimbalAng[0];

    double err = ref - angRoll;
    ...
    rollRate.data = 1.0*err;
    rollVelCommand_pub.publish(rollRate);
}

void GimbalControl::getUKFResults(const coverage::EstimateOutput::ConstPtr& data)
{
    ...
    if(objFind != 0 && objFind != NAN)
    {
        ...
        for(int i = 0; i < objLen; i++){
            ...
            if(info->bounding_boxes[i].Class == trackingTarget && info->bounding_boxes[i].probability >= legal_probability)
            {
                ...
                u = (info->bounding_boxes[i].xmax + info->bounding_boxes[i].xmin)/2;
                v = (info->bounding_boxes[i].ymax + info->bounding_boxes[i].ymin)/2;
                isDetected = true;
                // detectObj = info->bounding_boxes[0].Class;
                detectObj = info->bounding_boxes[i].Class;
...
{   
    ...
    int count = 0;
    int ckey = 0, old_ckey = 116-8+8;  // key (l)
    bool isUKFOn = 0;
    bool isYoloOn = 0;
    bool isDepthCalOn = 0;
