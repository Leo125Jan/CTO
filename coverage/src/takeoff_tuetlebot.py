...
if __name__ == '__main__':
    ...
    try:
        ...
        px4_7 = [Px4Controller(uavtype[0]), Px4Controller(uavtype[1]), Px4Controller(uavtype[2])]
        ...
        last_time = [rospy.Time.now(), rospy.Time.now(), rospy.Time.now()]

        while not rospy.is_shutdown():

            if rospy.Time.now() - last_time[0] > rospy.Duration(10):

                if not px4_7[0].current_state.armed or px4_7[0].current_state.mode != "OFFBOARD":

                	px4_7[0].OffboardandArm()
                	last_time[0] = rospy.Time.now()

            if rospy.Time.now() - last_time[1] > rospy.Duration(10):

                if not px4_7[1].current_state.armed or px4_7[1].current_state.mode != "OFFBOARD":

                	px4_7[1].OffboardandArm()
                	last_time[1] = rospy.Time.now()

            if rospy.Time.now() - last_time[2] > rospy.Duration(10):

                if not px4_7[2].current_state.armed or px4_7[2].current_state.mode != "OFFBOARD":

                    px4_7[2].OffboardandArm()
