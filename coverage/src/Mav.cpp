...
MAV::MAV(ros::NodeHandle &nh_, int ID)
{
    nh = nh_;
    self_name = "uav" + to_string(ID);
    ...
    state_sub = nh_.subscribe<gazebo_msgs::ModelStates>("/gazebo/model_states", 10, &MAV::state_CB, this);
...
MAV::MAV(ros::NodeHandle &nh_, string vehicle, int ID)
{
    ...
    self_name = vehicle + to_string(ID);

    if(id != 0)
    {
        // pose_sub = nh_.subscribe<geometry_msgs::PoseStamped>(prefix + string("/mavros/local_position/pose"), 10, &MAV::pose_cb, this);
        // vel_sub = nh_.subscribe<geometry_msgs::TwistStamped>(prefix + string("/mavros/local_position/velocity_local"), 10, &MAV::vel_cb, this);
        state_sub = nh_.subscribe<gazebo_msgs::ModelStates>("/gazebo/model_states", 10, &MAV::state_CB, this);
    ...
    else
    {
        ...
        pose_sub = nh_.subscribe<geometry_msgs::PoseStamped>("/uav5/mavros/local_position/pose", 10, &MAV::pose_cb, this);
        vel_sub = nh_.subscribe<geometry_msgs::TwistStamped>("/uav5/mavros/local_position/velocity_local", 10, &MAV::vel_cb, this);
    }
    mav_state_sub = nh_.subscribe<mavros_msgs::State>(prefix + string("/mavros/state"), 10, &MAV::mav_state_cb, this);
}

void MAV::state_CB(const gazebo_msgs::ModelStates::ConstPtr& msg) 
{
    gazebo_msgs::ModelStates state_ = *msg;
    
    auto it = find(state_.name.begin(), state_.name.end(), self_name);
    int index;

    // If element was found
    if (it != state_.name.end())
    {
        index = it - state_.name.begin();
    }
    else
    {
    ...
    pose_current.pose = state_.pose[index];
    vel_current.twist = state_.twist[index];
}
...
void MAV::pose_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    if(!pose_init)
        pose_init = true;
    pose_current = *msg;
    cout << pose_current.pose.position.x << " ," << pose_current.pose.position.y << " ," << pose_current.pose.position.z << endl;
}
