...
class UAV():
	...
	def Neighbor_Connection(self):
		...
		if (self.last_index is None):
			...
			ttopics_to_remove = set(topics_to_add) - set(topics_to_add)
...
if __name__ == '__main__':
	...
	try:
		...
		while (uav_2.agents_state is None) or (uav_2.agents_name is None) or\
				(uav_2.targets_state is None) or (uav_2.targets_name is None) or\
				(uav_2.Cams_state is None) or (uav_2.Cams_name is None) or\
				(uav_2.col_ind is None) or (uav_2.neighbor_index is None) or (uav_2.Pd is None) or\
				(uav_2.cluster_set is None) or (uav_2.teammate is None):
		...
		while not rospy.is_shutdown():
			...
			neighbor_index, teammate, varb_list_copy = uav_2.Neighbor_Connection()
			uav_2.Collect_Data(neighbor_index, teammate, varb_list_copy)
