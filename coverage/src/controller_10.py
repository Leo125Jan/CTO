...
def vel(msg):
	...
	if abs(msg.twist.linear.x) > 1e-6 and  abs(msg.twist.linear.y) > 1e-6:
		...
		currP10 = targets_state[id_]["position"]

def	controller(time_):

	global cmd_vel, currP10, targets_state, id_
	move_gain = 5
	delay_time = 1e-4

	u_des = np.array([1*((currP10[0] - targets_state[id_]["position"][0])),\
					1*((currP10[1] - targets_state[id_]["position"][1])),\
...
def Path_Follower(t):
	...
	if t < 20:
		...
		if targets_state[id_]["position"][0] >= 6.5 and targets_state[id_]["position"][1] >= 12.0:

			u_des = np.array([-10*(0.1), 2*(12.5-targets_state[id_]["position"][1]), 2*(9.8 - targets_state[id_]["position"][2])])
		...
		if targets_state[id_]["position"][0] < 12.0 and targets_state[id_]["position"][1] < 6.5:

			u_des = np.array([10*(0.1), 2*(6.0-targets_state[id_]["position"][1]), 9.8 - targets_state[id_]["position"][2]])

		if targets_state[id_]["position"][0] >= 12.0 and targets_state[id_]["position"][1] < 12.0:

			u_des = np.array([2*(12.5-targets_state[id_]["position"][0]), 10*(0.1), 9.8 - targets_state[id_]["position"][2]])
		u_mag = 0.45
	elif t < 50:
		u_des = 2.0*np.array([(targets_state[1]["position"][0]-targets_state[id_]["position"][0]) - 1.0 + targets_state[1]["velocity"][0],\
							(targets_state[1]["position"][1]-targets_state[id_]["position"][1]) - 1.0 + targets_state[1]["velocity"][1],\
							2*(9.8 - targets_state[id_]["position"][2])])
		u_mag = 0.5
	else:

		u_des = np.array([0.5*((targets_state[2]["position"][0]-targets_state[id_]["position"][0]) - 1.1) +\
							0.5*((targets_state[3]["position"][0]-targets_state[id_]["position"][0]) + 0.0),\
						0.5*((targets_state[2]["position"][1]-targets_state[id_]["position"][1]) + 0.0) +\
							0.5*((targets_state[3]["position"][1]-targets_state[id_]["position"][1]) - 1.1),\
						2*(9.8 - targets_state[id_]["position"][2])])
		u_mag = 0.5

	u_dir = u_des/np.linalg.norm(u_des)

	print("P9: ", targets_state[id_]["position"])
	print("P9 u_mag: ", u_mag)
	print("P9 u_dir: ", u_dir)
...
if __name__ == '__main__':
	...
	try:
		rospy.init_node('controller_10')
		px4 = Px4Controller("uav10")
		rospy.Subscriber('/gazebo/model_states', ModelStates, odom, queue_size=10)
		rospy.Subscriber('/uav10/cmd_vel', TwistStamped, vel, queue_size=10)
		rate = rospy.Rate(100)
		...
		currP9 = targets_state[id_]["position"]
