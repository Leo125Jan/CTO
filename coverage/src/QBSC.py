...
class PTZcon():
	...
	def CBAA(self, targets, time_):
		...
		if len(self.teammate) == 1:
			...
			print("CBAA id, one_hop_neighbor: ", + self.id, self.one_hop_neighbor)
			print("CBAA id, ResetComplete: ", self.id, self.neighbors["ResetComplete"])
			...
			print("CBAA id, TaskComplete: ", self.id, self.neighbors["TaskComplete"])
			...
			print("CBAA id, ResetReady: ", self.id, self.neighbors["ResetReady"])
			...
			print("CBAA id, Counting: ", self.id, self.neighbors["Counting"])
			...
			if np.sum(self.x) == 0:
				...
				self.c = np.linalg.norm(task-self.pos, axis=1)
				print("CBAA id, auction c: ", self.id, self.c)

				self.h = np.where(self.c < self.y, 1, 1e3)
				self.J = np.argmin(self.h*self.c)
				print("CBAA id, auction h: ", self.id, self.h)
				print("CBAA id, auction J: ", self.id, self.J)
				self.x[self.J] = 1
			...
			print("CBAA id, WinBid: ", self.id, self.neighbors["WinBid"])
	...
	def CBAA(self, targets, time_):
		...
		if len(self.teammate) == 1:
			...
			targets_position = np.array([targets[i][0] for i in range(len(targets))])
			task = targets_position[self.cluster_set]
			print("CBAA id, task: ", self.id, task)
			...
			for key in self.one_hop_neighbor:
				...
				if (self.neighbors["WinBid"][str(key)] != None):
					...
					if (len(self.neighbors["WinBid"][str(key)]) != len(c)):
			...
			if count > 0:
				...
				# print("CBAA id, cost_matrix: ", self.id, cost_matrix)

				row, col = linear_sum_assignment(cost_matrix)
				print("CBAA id, col: ", self.id, col)
				...
				print("CBAA id, missing_number: ", self.id, missing_numbers)
				...
				for (row, col) in zip(row, col):
					...
					col_sol[str(row)] = col
				print("CBAA id, col_sol: ", self.id, col_sol)
				...
				print("CBAA id, col_ind: ", self.id, col_ind)
			...
			# Winning Bid Publish
			output_ = Float64MultiArray(data = c)
			self.Winnig_Bid_pub.publish(output_)
	...
	def Graph_Construction(self, I_vertex):
		...
		print("id, I_vertex: ", self.id, I_vertex)
	...
	def Optimal_Cluster(self, targets, Pd, cluster_set, col_ind, cluster_labels, time_):
		...
		print("id, path_origin: ", self.id, path)
		print("id, trunk_hold_origin: ", self.id, trunk_hold)
		...
		print("id, path_combined: ", self.id, path)
		print("id, trunk_hold_origin: ", self.id, trunk_hold)

		# Sceondly Construction
		trunk_hold, path = self.Graph_Construction(path)
		print("id, path_evolution: ", self.id, path)
		print("id, trunk_hold_evolution: ", self.id, trunk_hold)

		# Secondly Judgement
		trunk_hold, path = self.Judgement_Algorithm(trunk_hold, path)
		print("id, path_finale: ", self.id, path)
		print("id, trunk_hold_finale: ", self.id, trunk_hold)
	...
	def Gradient_Descent(self, targets, Pd, cluster_set, col_ind, cluster_labels, time_):
		...
		incircle_A = 1.0*np.pi*incircle_r**2
		...
		print("id, path: ", self.id, str(path))
		print("id, trunk: ", self.id, str(trunk))
		target_points = []
		...
		if trunk[0][0] == -1:
			...
			if (np.shape(path)[0] > 1) and (self.Jx is not None):
				...
				print("id, nearest_index: ", self.id, nearest_index)
		...
		print("id, self_target: ", self.id, str(self.target))
