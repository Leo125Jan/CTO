...
def	controller(time_):
	...
	global cmd_vel, currP9, targets_state, id_
	move_gain = 5
	delay_time = 1e-4

	u_des = np.array([1*((currP9[0] - targets_state[id_]["position"][0])),\
					1*((currP9[1] - targets_state[id_]["position"][1])),\
...
if __name__ == '__main__':
	...
	try:
		rospy.init_node('controller_9')
		px4 = Px4Controller("uav9")
		rospy.Subscriber('/gazebo/model_states', ModelStates, odom, queue_size=10)
		rospy.Subscriber('/uav9/cmd_vel', TwistStamped, vel, queue_size=10)
		rate = rospy.Rate(100)
