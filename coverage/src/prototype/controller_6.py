...
def	controller(time_):
	...
	u_des = np.array([1*((currP6[0] - P6[0])), 1*((currP6[1] - P6[1])), 10 - P6[2]])
...
def Path_Follower(t):
	...
	if t < 20:
		...
		u_des = 1.0*np.array([(P5[0]-P6[0]) + 1.0 + P5v[0], (P5[1]-P6[1]) + 1.0 + P5v[1], 2*(9.8 - P6[2])])
		u_mag = 0.5
	else:

		if P6[0] <= 13.0 and P6[1] <= 7.5:

			u_des = np.array([10*(0.1), 2*(7.0-P6[1]), 2*(9.8 - P6[2])])

		if P6[0] > 13.0 and P6[1] <= 13.0:

			u_des = np.array([2*(13.5-P6[0]), 10*(0.1), 9.8 - P6[2]])

		if P6[0] > 7.5 and P6[1] > 13.0:

			u_des = np.array([-10*(0.1), 2*(13.5-P6[1]), 9.8 - P6[2]])

		if P6[0] <= 7.5 and P6[1] > 7.5:

			u_des = np.array([2*(7.0-P6[0]), -10*(0.1), 9.8 - P6[2]])
		u_mag = 0.4

	# CBF
	# neighbors_pos = np.array([P5, P7, P8, P9])
	# neighbors_vel = np.array([P5v, P7v, P8v, P9v])
	# dt_min = 0.4
	...
	# 	dist = np.linalg.norm(P6 - p)

	# 	if dist < communication_range:

	# 		d = (P6 - p)
	# 		h = 0.1*(dist**2 - dt_min**2) - 2*np.dot(d, v)
