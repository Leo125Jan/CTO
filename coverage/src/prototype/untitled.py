...
class UAV():
	...
	def Neighbor_Connection(self):
		...
		print("id, neighbor_index: ", self.id, self.neighbor_index)
		print("id, teammate: ", self.id, self.teammate)

		neighbor_topics = ["/uav" + str(index) for index in self.neighbor_index]
		...
		for neighbor_topic in neighbor_topics_to_remove:
			...
			for i, ref_topic in enumerate(topic_list):
				...
				self.remove_subscriber(str(neighbor_topic)+str(ref_topic), name, varb_list[i], sub_list[i])

		if len(self.teammate) > 1:

			topic_list = ["/targetResdiual", "/CurrVel", "/SweetSpot", "/WinBid", "/TaskComplete", "/ResetReady",\
							"/ResetComplete", "/Counting", "/Essential", "/Detection"]
			varb_list = [self.neighbors_residual, self.neighbors_vel, self.neighbors_swt, self.neighbors_winbid,\
			...
			sub_list = [self.neighbors_residual_sub, self.neighbors_vel_sub, self.neighbors_swt_sub, self.neighbors_winbid_sub,\
			...
			init_list = [np.array([[None, None]]), np.array([0.0, 0.0]), np.array([0.0, 0.0]), None, False, False, False, None, None, None]
			cb_list = [self.common_residual_callback, self.common_vel_callback, self.common_swt_callback, self.common_winbid_callback,\
			...
			topic_list = ["/targetResdiual", "/CurrVel", "/SweetSpot"]
			varb_list = [self.neighbors_residual, self.neighbors_vel, self.neighbors_swt]
			sub_list = [self.neighbors_residual_sub, self.neighbors_vel_sub, self.neighbors_swt_sub]
			init_list = [np.array([[None, None]]), np.array([0.0, 0.0]), np.array([0.0, 0.0])]
			cb_list = [self.common_residual_callback, self.common_vel_callback, self.common_swt_callback]
			...
			for i, ref_topic in enumerate(topic_list):
				...
				if ref_topic == "/Counting":
					...
					self.add_subscriber(str(neighbor_topic)+str(ref_topic), Int64, name, 50, 52428800,\
										varb_list[int(i)], sub_list[int(i)], init_list[int(i)], cb_list[int(i)])
				else:

					name = str(''.join(filter(str.isdigit, neighbor_topic)))
					self.add_subscriber(str(neighbor_topic)+str(ref_topic), Float64MultiArray, name, 50,\
										52428800, varb_list[int(i)], sub_list[int(i)], init_list[int(i)], cb_list[int(i)])

		print("id, varb_list: ", self.id, varb_list)
		print("id, sub_list: ", self.id, sub_list)
		# print("id, init_list: ", self.id, init_list)
		# print("id, cb_list: ", self.id, cb_list)

		self.last_neighbor_topics = current_topics_set
