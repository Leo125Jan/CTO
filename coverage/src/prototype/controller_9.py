...
def	controller(time_):
	...
	u_des = np.array([1*((currP9[0] - P9[0])), 1*((currP9[1] - P9[1])), 9.8 - P9[2]])
...
def Path_Follower(t):
	...
	if t < 20:
		...
		if P9[0] >= 6.5 and P9[1] >= 12.0:

			u_des = np.array([-10*(0.1), 2*(12.5-P9[1]), 2*(9.8 - P9[2])])

		if P9[0] < 6.5 and P9[1] >= 6.5:

			u_des = np.array([2*(6.0-P9[0]), -10*(0.1), 9.8 - P9[2]])

		if P9[0] < 12.0 and P9[1] < 6.5:

			u_des = np.array([10*(0.1), 2*(6.0-P9[1]), 9.8 - P9[2]])

		if P9[0] >= 12.0 and P9[1] < 12.0:

			u_des = np.array([2*(12.5-P9[0]), 10*(0.1), 9.8 - P9[2]])
		u_mag = 0.45
	elif t < 50:
		u_des = 2.0*np.array([(P6[0]-P9[0]) - 1.0 + P6v[0], (P6[1]-P9[1]) - 1.0 + P6v[1], 2*(9.8 - P9[2])])
		u_mag = 0.5
	else:

		u_des = np.array([0.5*((P7[0]-P9[0]) - 1.1) + 0.5*((P8[0]-P9[0]) + 0.0),\
						0.5*((P7[1]-P9[1]) + 0.0) + 0.5*((P8[1]-P9[1]) - 1.1),\
						2*(9.8 - P9[2])])
		u_mag = 0.5
