...
class UAV():
	...
	def __init__(self, id_):
		...
		self.img1, self.pixel1, self.camerainfo1 = np.array([[0.0,0.0,0.0]]), np.array([[0.0,0.0,0.0]]), np.array([[0.0,0.0,0.0,0.0]])
		...
		self.Ref_sub = rospy.Subscriber('/uav1/Ref', Float64MultiArray, self.reference_callback, queue_size = 20, buff_size = 52428800)
	...
	def Recognition1_callback(self, msg):
		...
		if len(pixel1) == n:
			...
			self.pixel1 = np.array(pixel1)
			self.img1 = np.array(img1)
		elif len(pixel1) < n and not np.array_equal(self.pixel1, np.array([[0.0,0.0,0.0]])) \
								and not np.array_equal(self.img1, np.array([[0.0,0.0,0.0]])):
	...
	def Collect_Data(self):
		...
		self.current_heading = self.q2yaw(self.P1o)
		...
		self.states = {"Position": self.pos, "Perspective": self.perspective, "NeighorIndex": self.neighbor_index,\
						...
						"CamPos": self.Cam1, "CamOrientation": self.Cam1o,\
						"img": self.img1, "pixel": self.pixel1, "camerainfo": self.camerainfo1}

		# Neighbors States
		neighbors_position = {"0": [self.P0[0], self.P0[1]], "2": [self.P2[0], self.P2[1]],\
								"3": [self.P3[0], self.P3[1]], "4": [self.P4[0], self.P4[1]]}
		neighbors_Cam_position = {"0": self.Cam0, "2": self.Cam2, "3": self.Cam3, "4": self.Cam4}
		neighbors_Cam_Orientation = {"0": self.Cam0o, "2": self.Cam2o, "3": self.Cam3o, "4": self.Cam4o}

		# print("Neighbor0_residual: ", self.neighbor0_residual)
		# print("Neighbor2_residual: ", self.neighbor2_residual)
		neighbors_residual = {"0": self.neighbor0_residual, "2": self.neighbor2_residual,\
								"3": self.neighbor3_residual, "4": self.neighbor4_residual}
		neighbors_vel = {"0": self.neighbor0_vel, "2": self.neighbor2_vel, "3": self.neighbor3_vel, "4": self.neighbor4_vel}
		neighbors_id = self.neighbor_index
		neighbors_swt = {"0": self.neighbor0_swt, "2": self.neighbor2_swt, "3": self.neighbor3_swt, "4": self.neighbor4_swt}
		neighbors_winbid = {"0": self.neighbor0_winbid, "2": self.neighbor2_winbid, "3": self.neighbor3_winbid, "4": self.neighbor4_winbid}
		neighbors_taskcomplete = {"0": self.neighbor0_taskcomplete, "2": self.neighbor2_taskcomplete, "3": self.neighbor3_taskcomplete,\
									"4": self.neighbor4_taskcomplete}
		neighbors_resetready = {"0": self.neighbor0_resetready, "2": self.neighbor2_resetready, "3": self.neighbor3_resetready,\
								"4": self.neighbor4_resetready}
		neighbors_resetcomplete = {"0": self.neighbor0_resetcomplete, "2": self.neighbor2_resetcomplete, "3": self.neighbor3_resetcomplete,\
									"4": self.neighbor4_resetcomplete}
		neighbors_counting = {"0": self.neighbor0_counting, "2": self.neighbor2_counting, "3": self.neighbor3_counting,\
								"4": self.neighbor4_counting}
		neighbors_Essential = {"0": self.neighbor0_ess, "2": self.neighbor2_ess, "3": self.neighbor3_ess, "4": self.neighbor4_ess}
		neighbors_Detection = {"0": self.neighbor0_det, "2": self.neighbor2_det, "3": self.neighbor3_det, "4": self.neighbor4_det}
	...
	def	controller(self, dx, dp, step):
		...
		self.cmd_vel.linear.z = 11.0 - self.P1[2]
...
if __name__ == '__main__':
	...
	try:
		...
		ptz_1 = PTZcon(camera1, map_size, grid_size)
		uav_1 = UAV(camera1["id"])
		# est_1 = DepthEstimator(camera1['ecode'], "/home/leo/mts/src/coverage/record/uav1.csv",\
		# 					uav_1.targetName, uav_1.pixel_size, uav_1.image_width, uav_1.FL_Curr)

		# filename = "/home/leo/mts/src/coverage/record/uav1.csv"
		# f = open(filename, "w+")
		# f.close()

		while (uav_1.P1 is None) or (uav_1.neighbor_index is None):
		...
		while not rospy.is_shutdown():
			...
			# est_1.MultiTarget_Estimation(uav_1.states, uav_1.neighbors)
			# uav_1.SetCameraZoom(1)
