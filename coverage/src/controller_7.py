...
def vel(msg):
	...
	if abs(msg.twist.linear.x) > 1e-6 and  abs(msg.twist.linear.y) > 1e-6:
		...
		currP7 = targets_state[id_]["position"]

def	controller(time_):

	global cmd_vel, currP7, targets_state, id_
	move_gain = 5
	delay_time = 1e-4

	u_des = np.array([1*((currP7[0] - targets_state[id_]["position"][0])),\
						1*((currP7[1] - targets_state[id_]["position"][1])),\
...
def Path_Follower(t):
	...
	if t < 20:
		...
		u_des = 2.0*np.array([(targets_state[4]["position"][0]-targets_state[id_]["position"][0])-1.3+targets_state[4]["velocity"][0] +\
								(targets_state[3]["position"][0]-targets_state[id_]["position"][0]),\
							(targets_state[4]["position"][1]-targets_state[id_]["position"][1])-1.0+targets_state[4]["velocity"][1] +\
								(targets_state[3]["position"][1]-targets_state[id_]["position"][1])-2,\
							2*(9.8 - targets_state[id_]["position"][2])])
		u_mag = 0.5
	else:

		if targets_state[id_]["position"][0] <= 17.0 and targets_state[id_]["position"][1] <= 6.0:

			u_des = np.array([10*(0.1), 2*(5.5-targets_state[id_]["position"][1]), 2*(9.8 - targets_state[id_]["position"][2])])

		if targets_state[id_]["position"][0] > 17.0 and targets_state[id_]["position"][1] <= 17.0:

			u_des = np.array([2*(17.5-targets_state[id_]["position"][0]), 10*(0.1), 9.8 - targets_state[id_]["position"][2]])

		if targets_state[id_]["position"][0] > 6.0 and targets_state[id_]["position"][1] > 17.0:

			u_des = np.array([-10*(0.1), 2*(17.5-targets_state[id_]["position"][1]), 9.8 - targets_state[id_]["position"][2]])

		if targets_state[id_]["position"][0] <= 7.5 and targets_state[id_]["position"][1] > 6.0:
	...
	print("P7: ", targets_state[id_]["position"])
	print("P7 u_mag: ", u_mag)
	print("P7 u_dir: ", u_dir)
...
if __name__ == '__main__':
	...
	try:
		rospy.init_node('controller_7')
		px4 = Px4Controller("uav7")
