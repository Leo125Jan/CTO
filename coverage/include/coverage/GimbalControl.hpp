...
#include <coverage/Dynamics.h>
#include <coverage/CameraFeatures.h>
#include <coverage/EstimateOutput.h>
...
private:
    ...
    float angPitch = 0, angYaw = 0, angRoll = 0;

    /*** estimate ***/
    coverage::Dynamics dyn_value;
    coverage::CameraFeatures cam_value;
    float ukf_x1, ukf_x2;
...
public:
    GimbalControl(std::string self_name, std::string target_name);;
    ~GimbalControl();
    ...
    void getUKFResults(const coverage::EstimateOutput::ConstPtr& data);
    void calRelPoseUKF();
